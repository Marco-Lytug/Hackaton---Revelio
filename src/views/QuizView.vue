...
const perguntas = computed(() => buscarPerguntas(curso.value, ano.value, idQuiz.value))

const tituloQuiz = computed(() => buscarTituloQuiz(curso.value, ano.value, idQuiz.value))
...
<template>
  ...
  >
    ...
    <section class="quiz-area">
      <Quiz :titulo="tituloQuiz" :perguntas="perguntas" @finalizar="aoFinalizarQuiz" />
    </section>
  </div>
</template>
...
.quiz-topo {
  ...
  background: linear-gradient(135deg, var(--cor-curso), var(--cor-hover));
...
.quiz-area {
  ...
  background: linear-gradient(to bottom, var(--cor-clara), #ffffff 300px);
}
