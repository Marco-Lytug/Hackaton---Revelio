...
const todasCategorias = computed(
  () => categoriaInfo.value || categoriaAgro.value || categoriaQuimi.value,
)
...
const FiltroAutorTitulo = computed(() => {
  if (filtro.value.trim().length > 0) {
    return LivrosExibidos.value.filter(
      (item) =>
        item.autor.toLowerCase().includes(filtro.value.toLowerCase()) ||
        item.titulo.toLowerCase().includes(filtro.value.toLowerCase()),
    )
...
<template>
  <section class="pesquisa">
    <div class="barras">
      <div class="barra-pesquisa">
        <input
          class="input"
          type="text"
          placeholder="Pesquisar por título ou por autor"
          v-model="filtro"
        />

        <div
          class="select"
          :class="{ ativo: props.categorias === 'info' }"
          v-if="props.categorias === 'info'"
        >
        ...
        <div
          class="select"
          :class="{ ativo: props.categorias === 'agro' }"
          v-if="props.categorias === 'agro'"
        >
        ...
        <div
          class="select"
          :class="{ ativo: props.categorias === 'quimi' }"
          v-if="props.categorias === 'quimi'"
        >
    ...
    <div class="livro-lista" :class="lista">
      <LivroCard
        v-for="livros in FiltroAutorTitulo"
        :key="livros.id"
        class="Livro-card"
        :id="livros.id"
